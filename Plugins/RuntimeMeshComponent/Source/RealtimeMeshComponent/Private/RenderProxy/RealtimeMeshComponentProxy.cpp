...
{	
...
#endif
	{
		...
		{			
			...
			if (RayTracingGeometry->Initializer.TotalPrimitiveCount <= 0 || !RayTracingGeometry->Initializer.IndexBuffer.IsValid())
			{
				continue;
			}
			
#if RMC_ENGINE_ABOVE_5_5
			if (!RayTracingGeometry->GetRHI() || !RayTracingGeometry->IsValid())
			{
				continue;
			}
			FRayTracingInstance RayTracingInstance;
#else
			if (!RayTracingGeometry->RayTracingGeometryRHI || !RayTracingGeometry->IsValid())
			{
				continue;
			}
