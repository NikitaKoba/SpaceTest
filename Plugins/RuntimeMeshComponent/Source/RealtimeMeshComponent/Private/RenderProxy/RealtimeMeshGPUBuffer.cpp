...
{
	static TAutoConsoleVariable<int32> CVarRealtimeMeshAsyncBufferCreate(
		TEXT("r.RealtimeMesh.AsyncBufferCreate"),
		0,
		TEXT("Create realtime mesh GPU buffers on the async RHI command list when possible."));

	void FRealtimeMeshSectionGroupStreamUpdateData::CreateBufferAsyncIfPossible(FRealtimeMeshUpdateContext& UpdateContext)
	{
		if (CVarRealtimeMeshAsyncBufferCreate.GetValueOnAnyThread() == 0)
		{
			return;
		}
